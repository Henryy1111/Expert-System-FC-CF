...
export default function BobotCFIndex() {
  ...
  const fetchBobotCF = async () => {
    ...
    const { data, error } = await supabase
      .from("bobotcf")
      .select("id_bobotcf, bobotcf, nilaicf")
      .order("nilaicf", { ascending: false });
  ...
  const handleDelete = async () => {
    const { error } = await supabase
      .from("bobotcf")
      .delete()
      .eq("id_bobotcf", selectedId);
    setShowModal(false);
    if (error) {
      showToast("error", "Gagal menghapus data");
    } else {
      showToast("success", "Bobot berhasil dihapus");
      fetchBobotCF();
    }
  };

  const filteredData = data.filter((item) =>
    item.bobotcf.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <AdminLayout>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-12">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1 font-medium italic">
              Certainty Factor Levels Configuration
            </p>
          </div>
        ...
        <div className="bg-slate-200 dark:bg-gray-900 rounded-[2.5rem] shadow-xl border border-gray-100 dark:border-gray-800 overflow-hidden">
                      ...
                      >
                        ...
                        <td className="px-8 py-6">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-gray-800 dark:text-gray-100 font-black text-lg tracking-tight">
                              {item.bobotcf}
                            </span>
                          </div>
                        </td>
                        <td className="px-8 py-6">
                          <div className="flex justify-center">
                            <div className="font-mono text-lg font-black text-indigo-600 bg-white dark:bg-indigo-900/30 px-5 py-2 rounded-2xl border border-indigo-100 dark:border-indigo-800 shadow-sm">
                              {item.nilaicf}
                            </div>
                          </div>
                        </td>
                        <td className="px-8 py-6">
                          {/* TOMBOL AKSI: SELALU TAMPIL & BERSAMPINGAN */}
                          <div className="flex justify-center items-center gap-3">
                            <Link
                              to={`/bobotcf/edit/${item.id_bobotcf}`}
                              className="flex items-center gap-2 px-5 py-2.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-indigo-600 hover:bg-indigo-600 hover:text-white hover:shadow-lg transition-all font-bold text-sm"
                            >
                              <Pencil className="w-4 h-4" /> Edit
                            </Link>
                            <button
                              onClick={() => confirmDelete(item.id_bobotcf)}
                              className="flex items-center gap-2 px-5 py-2.5 bg-rose-50 dark:bg-rose-900/20 text-rose-600 rounded-xl hover:bg-rose-600 hover:text-white hover:shadow-lg transition-all font-bold text-sm"
                            >
          ...
          {/* MOBILE LIST (SAMA DENGAN RULE INDEX) */}
            ...
            {filteredData.map((item) => (
              ...
              >
                <div className="flex justify-between items-center mb-4">
                  <div className="px-3 py-1 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 font-mono text-xs font-bold rounded-full border border-indigo-100">
                    VAL: {item.nilaicf}
                  </div>
                </div>
                <h3 className="text-xl font-black text-gray-800 dark:text-white mb-6 uppercase italic">
                  {item.bobotcf}
                </h3>
        ...
        <div className="mt-12 flex flex-col sm:flex-row items-center justify-between gap-6 border-t border-gray-100 dark:border-gray-800 pt-8">
          ...
          >
            <div className="w-10 h-10 rounded-full bg-gray-50 dark:bg-gray-900 flex items-center justify-center group-hover:bg-indigo-50 transition-colors border border-gray-100 dark:border-gray-800">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-400 font-medium">
            ...
            {filteredData.length} Data Certainty Factor (CF)
          </div>
        </div>
      </div>
    </AdminLayout>
